...
import newsletterSignupPrompt from './modules/newsletterSignupPrompt';
import {
  EXPERIENCE_ID_PROD_REG_GATE,
  EXPERIENCE_ID_SANDBOX_REG_GATE,
...
const gptInit = async (consent, hasRegGate = 0) => {
  await gpt({
    refresh,
...
const initialisers = [
  ...
  {
    userStatus: () => {
      // Maybe we need to wait for userStatus' resolving
      JSGlobals.userLogin && JSGlobals.article && userStatus();
    },
    gpt:
      !inCMSView &&
      JSGlobals.article &&
      (async () => {
        ...
        // Made for this ticket: https://app.clickup.com/t/4521779/COM-1493
        window.tp.push([
          'addHandler',
          'experienceExecute',
          async ({ result: { events } }) => {
            const regGateExperienceId =
              JSGlobals.mode === 'development'
                ? EXPERIENCE_ID_SANDBOX_REG_GATE
                : EXPERIENCE_ID_PROD_REG_GATE;
            const regGateEvent = events.find(
              ({ eventType, eventExecutionContext: { experienceId } }) =>
                eventType === 'showTemplate' &&
                experienceId === regGateExperienceId,
            );

            await gptInit(consent, regGateEvent ? 1 : 0);
          },
        ]);
      }),
  },
  ...
  },
  {
    ...
    userStatus: JSGlobals.userLogin && !JSGlobals.article && userStatus,
    taboola,
  },
];
