...
function populatePermutiveIDInput(value) {
  const input = form.querySelector('[name=permutiveID]');
  if (!input) {
    console.error(
      'Can not find input for permutiveID. Check the markup in RegisterForm.',
    );
  }
  input.value = value;
}

permutiveReady(populatePermutiveIDInput);
