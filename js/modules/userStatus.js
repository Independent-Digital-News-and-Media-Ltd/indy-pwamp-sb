...
export const articleIsOverOneYearOld = (todayDate, articleDate) => {
  return moment(todayDate).subtract(1, 'years').isAfter(articleDate, 'day');
};
...
export default async () => {
  ...
  const pianoCallback = async () => {
    ...
    if (JSGlobals.article) {
      ...
      if (articleIsOverOneYearOld(today, JSGlobals.article.publish)) {
        tags.push('archive');
      }
