...
const SUBSCRIPTION_CAUSE_KEY = 'subscription-cause';
...
const saveSubscriptionCause = () => {
  const { articleUrl } = window.JSGlobals?.taboola;
  if (articleUrl) {
    sessionStorage.setItem(SUBSCRIPTION_CAUSE_KEY, articleUrl);
  }
};

export const trackSubscriptionCause = () => {
  const articleUrl = sessionStorage.getItem(SUBSCRIPTION_CAUSE_KEY);
  if (articleUrl) {
