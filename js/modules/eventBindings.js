...
const eventArticleComplete = () => {
  ...
  if (main) {
    ...
    const reachArticleBottom = () =>
      window.scrollY + window.innerHeight >= main.clientHeight + main.offsetTop;
    const listener = () => {
      if (!ticking) {
...
const eventArticleBookmark = () => {
  const buttons = document.querySelectorAll('.social-share [type="bookmark"]');
  buttons?.forEach((btn) => {
    addPassiveEventListener(btn, 'click', () => dispatchArticleBookmark());
  });
};

const eventDonationButtonClick = () => {
  const buttons = document.querySelectorAll('.donation-btn');
  buttons?.forEach((btn) => {
    addPassiveEventListener(btn, 'click', () => dispatchDonationButtonClick());
  });
};
...
const eventGalleryVIew = () => {
  ...
  galleries?.forEach((gallery) => {
    addPassiveEventListener(gallery, 'click', () => dispatchGalleryView());
  });
};
...
export default () => {
  ...
  eventArticleBookmark();
  eventDonationButtonClick();
  eventGalleryVIew();
  eventAutoRenewalOff();
  eventOpenLogin();
  eventArticleShare();
