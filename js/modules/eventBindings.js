...
import { storeDigitalData } from './digitalData';
...
const eventSubscribeOnSellingPage = () => {
  document.addEventListener(EVENT_PIANO_READY, () => {
    window.addEventListener('message', (event) => {
      if (event.data && event.data.sellingPageData) {
        const { sellingPageData } = event.data;
        ...
        window.tp.push([
          ...
          () => {
            ...
            showPianoOffer(
              sellingPageData.offerId,
              sellingPageData.termId,
              trackingData,
            );
          },
        ]);
