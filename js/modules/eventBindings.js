...
const eventCloseRegistrationWall = () => {
  document.addEventListener(EVENT_PIANO_READY, () => {
    window.tp = window.tp || [];
    ...
    window.tp.push([
      ...
      ({ eventName }) => {
        if (eventName === 'close-registration-wall') {
          ...
          articleContainer.classList.remove('limited-access');
        }
      },
    ]);
  });
...
const eventNewsletterLoggedInFormClick = () => {
  const form = document.getElementById('reg-lite--form-logged-in');

  if (form) {
    const keyInput = form.querySelector('.newsletter-key');

    if (keyInput) {
      const key = keyInput.name;

      const newsletter = getNewsletterName(key);
      const signupButton = form.querySelector('.signup-btn');
...
export default () => {
  ...
  eventCloseRegistrationWall();
  eventShowCommentsClick();
  eventNewsletterPrefsCheckboxToggle();
  eventNewsletterLoggedInFormClick();
};
