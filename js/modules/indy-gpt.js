...
const addTopicsTargeting = () => {
  const topicsElem = document.getElementsByClassName('topics')[0];
  const sectionTopicUnfiltered = document.location.pathname
    .substring(1)
    .split('/', 3);
  const sectionTopic = sectionTopicUnfiltered.filter(
    (topic) => topic !== 'topic',
  );
  const topics = [
    ...[...(topicsElem?.getElementsByTagName('a') || [])].map(
      (el) => el.innerText,
    ),
    ...(sectionTopic ? [sectionTopic.map((topic) => 's_'.concat(topic))] : []),
  ];
